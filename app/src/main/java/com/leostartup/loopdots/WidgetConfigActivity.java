...
public class WidgetConfigActivity extends Activity {
    ...
    public static boolean roundDots(Context c,int id){return c.getSharedPreferences(PREFS,0).getBoolean("round_dots_"+id,false);}
    ...
    public static void clear(Context c,int id){android.content.SharedPreferences.Editor e=c.getSharedPreferences(PREFS,0).edit();for(String prefix:new String[]{"widget_","multi_","month_","glass_","alpha_","tone_","dot_size_","text_size_","background_","round_dots_"})e.remove(prefix+id);e.apply();}
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);setResult(RESULT_CANCELED);widgetId=getIntent().getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID,-1);if(widgetId==-1){finish();return;}
        ...
        root.addView(label("Formato das marcas",16));RadioGroup shape=new RadioGroup(this);RadioButton squares=new RadioButton(this);squares.setId(android.view.View.generateViewId());squares.setText("Quadradinhos com cantos arredondados");squares.setTextColor(Color.WHITE);shape.addView(squares);RadioButton circles=new RadioButton(this);circles.setId(android.view.View.generateViewId());circles.setText("Bolas");circles.setTextColor(Color.WHITE);shape.addView(circles);shape.check(roundDots(this,widgetId)?circles.getId():squares.getId());root.addView(shape);
        ...
        Runnable preview=()->{getSharedPreferences(PREFS,0).edit().putInt("background_"+widgetId,chosenBg[0]).putInt("alpha_"+widgetId,alpha.getProgress()).putInt("dot_size_"+widgetId,dot.getProgress()+70).putInt("text_size_"+widgetId,text.getProgress()+70).putBoolean("round_dots_"+widgetId,shape.getCheckedRadioButtonId()==circles.getId()).apply();LoopDotsWidgetProvider.updateWidget(this,AppWidgetManager.getInstance(this),widgetId);};
        SeekBar.OnSeekBarChangeListener listener=new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar s,int p,boolean user){aLabel.setText("Opacidade: "+alpha.getProgress()+"%");dLabel.setText("Tamanho das marcas: "+(dot.getProgress()+70)+"%");tLabel.setText("Tamanho do texto: "+(text.getProgress()+70)+"%");if(user)preview.run();}public void onStartTrackingTouch(SeekBar s){}public void onStopTrackingTouch(SeekBar s){preview.run();}};alpha.setOnSeekBarChangeListener(listener);dot.setOnSeekBarChangeListener(listener);text.setOnSeekBarChangeListener(listener);shape.setOnCheckedChangeListener((g,checked)->preview.run());bg.setOnCheckedChangeListener((g,checked)->{for(int i=0;i<bgButtons.length;i++)if(bgButtons[i].getId()==checked)chosenBg[0]=i;preview.run();});
